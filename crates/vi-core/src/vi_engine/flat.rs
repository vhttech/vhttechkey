...
use crate::vi_engine::types::{
    EngineMode, EffectType, ENGLISH_MODE, LOWERCASE_MODE, MARK_LESS, Mark, Trans, TONE_LESS,
};

pub(crate) fn flatten(composition: &[Trans], mode: EngineMode) -> String {
    get_canvas(composition, mode).into_iter().collect::<String>().nfc().collect::<String>()
}
...
pub(crate) fn get_canvas(composition: &[Trans], mode: EngineMode) -> Vec<char> {
    ...
    for trans in composition {
        ...
        if mode & ENGLISH_MODE != 0 {
            ...
            appending_map.entry(ptr_usize(targ)).or_default().push(trans.clone());
        }
    }
    ...
    for appending_trans in appending_list {
        ...
        let trans_list = appending_map.remove(&ptr_usize(&appending_trans)).unwrap_or_default();
